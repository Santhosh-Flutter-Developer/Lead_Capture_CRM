...
final ScrollController _scrollController = ScrollController();
  ...
  Widget build(BuildContext context) {
    final controllerRead =
        context.read<PaginatedDataController<DepartmentModel>>();
    final controllerWatch =
        context.watch<PaginatedDataController<DepartmentModel>>();
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  "Organize your teams into clear, manageable departments",
    ...
    final buttons = <Widget>[
      (permissions?.canCreate ?? false)
          ? _gradientButton(
              ...
              onPressed: () {
                if (kIsMobile || !isWide) {
                  Sheet.showSheet(context, widget: const DepartmentCreate());
                } else {
      ...
      if (_selectedDepartments.isNotEmpty && (permissions?.canDelete ?? false))
        _gradientButton(
          context: context,
          icon: Iconsax.trash,
          label: "Delete (${_selectedDepartments.length})",
  ...
  Widget _searchBox() {
    return _DepartmentSearchField(
      pageTitle: _pageTitle,
      onChanged:
          context.read<PaginatedDataController<DepartmentModel>>().setSearch,
    );
  }
  ...
  DataRow _buildDataRow(
    ...
    final isSelected = controllerWatch.selectedIds.contains(department.uid);
    return DataRow(
      selected: isSelected,
      ...
      cells: [
        DataCell(
          Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              _avatarCircle(department.name, size: 32),
        ...
        DataCell(
          ConstrainedBox(
            ...
            child: Text(
              department.description.isNotEmpty ? department.description : "—",
        ...
        DataCell(
          Row(
            ...
            children: [
              ...
              const SizedBox(width: 6),
              Text(
                department.createdAt.listingDateTime,
                style: Theme.of(context).textTheme.bodySmall,
              ),
            ],
          ),
  ...
  // Actions
  // ---------------------------------------------------------------------
  void _onEditTap(DepartmentModel department) {
    final isWide = _isWide(context);
    if (!isWide) {
      Sheet.showSheet(
        context,
  ...
  Future<void> _onDeleteTap(DepartmentModel department) async {
    ...
    try {
      ...
      await DepartmentService.deleteDepartment(uid: department.uid ?? '');
      ...
      FlushBar.show(
        ...
        onActionPressed: () async {
          ...
          await DepartmentService.restoreDepartment(deletedDepartment);
          if (!mounted) return;
          context.read<DepartmentBloc>().add(StreamDepartment());
        },
      );
  ...
  Future<void> _bulkDelete() async {
    ...
    try {
      final deleted = _selectedDepartments.map((e) => e.copyWith()).toList();

      futureLoading(context);

      for (var department in deleted) {
        await DepartmentService.deleteDepartment(uid: department.uid ?? '');
      }
      ...
      FlushBar.show(
        ...
        onActionPressed: () async {
          for (var department in deleted) {
            if (department.uid == null) continue;
            await DepartmentService.restoreDepartment(department);
          }
          if (!mounted) return;
          context.read<DepartmentBloc>().add(StreamDepartment());
        },
...
// ---------------------------------------------------------------------
// A more polished, colorful search field for the Department toolbar.
// ---------------------------------------------------------------------
class _DepartmentSearchField extends StatefulWidget {
  final String pageTitle;
  final ValueChanged<String> onChanged;
  const _DepartmentSearchField({
    required this.pageTitle,
    required this.onChanged,
  });

  @override
  State<_DepartmentSearchField> createState() => _DepartmentSearchFieldState();
}

class _DepartmentSearchFieldState extends State<_DepartmentSearchField> {
